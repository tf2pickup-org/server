...
function makeAllPossibleLineups(
  ...
  for (const gameClass of gameClasses) {
    ...
    if (playersOfGameClass.length === 2) {
      ...
      throw new NotImplementedException(
        'more than two players of one class in a team is not implemented',
      );
    }
  }
  ...
  function makeLineup(
    prev: PossibleLineup = { 0: { lineup: [] }, 1: { lineup: [] } },
    i = 0,
  ) {
...
function calculateAverageSkillDifference(
  ...
  return {
    ...lineup,
    skillAverageDifference: Math.abs(
      lineup[0].skillAverage - lineup[1].skillAverage,
    ),
  };
}

function respectsOverrides(
  lineup: Record<TeamId, TeamLineup>,
