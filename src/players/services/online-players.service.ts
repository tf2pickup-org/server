  ...
  onModuleInit() {
    this.playersGateway.playerConnected.subscribe(socket => {
      if (socket.request.user.logged_in) {
        ...
        const sockets = this.sockets.get(player._id.toString()) || [];
        if (!sockets.includes(socket)) {
          this.logger.debug(`${player.name} connected`);
          this.sockets.set(player._id.toString(), [ ...sockets, socket ]);
        }
      }
    });
    ...
    this.playersGateway.playerDisconnected.subscribe(socket => {
      if (socket.request.user.logged_in) {
        ...
        const sockets = this.getSocketsForPlayer(player._id.toString());
        this.sockets.set(player._id.toString(), sockets.filter(s => s !== socket));
        setTimeout(() => this.verifyPlayer(player._id.toString()), this.verifyPlayerTimeout);
      }
    });
  }
