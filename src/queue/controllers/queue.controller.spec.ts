  ...
  describe('#getMaps()', () => {
    beforeEach(() => {
      mapPoolService.getMaps.mockResolvedValue([
        new MapPoolItem('cp_badlands', 'etf2l_6v6_5cp'),
      ]);
    });
  ...
  describe('#addMap()', () => {
    beforeEach(() => {
      mapPoolService.addMap.mockImplementation((map) =>
        Promise.resolve(new MapPoolItem(map.name, map.execConfig)),
      );
    });
  ...
  describe('#deleteMap()', () => {
    beforeEach(() => {
      mapPoolService.removeMap.mockImplementation((name) =>
        Promise.resolve(new MapPoolItem(name, 'etf2l_6v6_5cp')),
      );
    });
  ...
  describe('#setMaps()', () => {
    ...
    it('should set the maps', async () => {
      const ret = await controller.setMaps([
        ...
        new MapPoolItem('cp_process_final', 'etf2l_6v6_5cp'),
      ]);
      expect(ret).toEqual([
        { name: 'cp_badlands' },
